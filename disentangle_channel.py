...
frames = np.concatenate(frames, axis=0)-neutral
topc = np.argsort(-np.var(frames,axis=0))[:n]
print("var=", np.var(frames,axis=0)[topc])

frame_paths = sorted(glob(f"logs/A00026/inference/predict*.pt"))
frames2=[]
for frame_path in frame_paths:
    frames2.append(torch.load(frame_path).detach().cpu().numpy().reshape(-1, 512*18))
frames2 = np.concatenate(frames2, axis=0)-neutral
topc2 = np.argsort(-np.var(frames2,axis=0))[:n]
print("var2=", np.var(frames2,axis=0)[topc2])
