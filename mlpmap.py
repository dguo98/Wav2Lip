...
class Audio2FrameDataset(object):
    def __init__(self, args, path, split, load_img=False, cache=True):
        ...
        self.latent_files = sorted(glob(f"{path}/frame_latents/latents_[0123456789]*"))
        assert len(self.audio_files) == len(latent_files)
        ...
        if self.cache:
            ...
            for i, f in enumerate(self.audio_files):
                v = torch.load(f).reshape(-1).cpu().numpy()
            ...
            if self.split is not "test":
                self.latent_vecs = []
                for i, f in enumerate(self.latent_files):
...
def train(args, train_loader, model, optimizer, neutral_vec):
    model.train()
...
def evaluate(args, data_loader, model, neutral_vec):
    model.eval()
...
def inference(args, data_loader, model, infer_dir):
     model.eval()
    ...
    for n_iter, (ids, audio_vec, latent_vec) in tqdm(enumerate(data_loader),total=total, desc="inference"):
        ...
        for i in range(bsz):
            idx = ids[i].item()
            torch.save(predict_vec, f"{infer_dir}/predict_{idx:04d}.pt")
...
if __name__ == "__main__":
    ...
    neutral_vec = train_dataset.get_neutral().to(device)
    ...
    neutral_vec = test_dataset.get_neutral().to(device)
    inference(args, test_data_loader, model, infer_dir)
