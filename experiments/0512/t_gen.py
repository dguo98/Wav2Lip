...
def generate(gpu, bs,warmup,epochs,h,d_model,d_ff,dp,n,seq_len,use_pose,
    template, img_type, img_loss, img_mouth, lmk_loss):
    global expid
    expid += 1
...
if __name__ == "__main__":
    ...
    # whether to use img_mouth
    for _img_mouth in [1]:
        for _img_loss in [0.2, 0.8]:
            scripts.append(generate(gpu,bs,warmup,epochs,h,d_model,d_ff,dp,n,seq_len, use_pose,template,img_type,_img_loss,_img_mouth, lmk_loss))
    
    
    # BUG (didn't actually run landmark loss!!)
    epochs=10
    _img_loss = 0.0
    for _img_mouth in [1]:
        for _lmk_loss in [0.5, 1.0]:
            scripts.append(generate(gpu,bs,warmup,epochs,h,d_model,d_ff,dp,n,seq_len, use_pose,template,img_type,img_loss,_img_mouth, _lmk_loss))

    epochs=1
    for _img_mouth in [0, 1]:
        for _img_loss in [1.0]:
            scripts.append(generate(gpu,bs,warmup,epochs,h,d_model,d_ff,dp,n,seq_len, use_pose,template,img_type,_img_loss,_img_mouth, lmk_loss))
