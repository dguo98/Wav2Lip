...
class Audio2FrameDataset(object):
    ...
    def __getitem__(self, idx):
        if self.args.mode == "default":
            ...
            assert self.args.mode == "diff"
            latent_vec = self.latent_vecs[idx]
...
def inference(args, data_loader, model, infer_dir, neutral_vec):
    ...
    for n_iter, (ids, audio_vec, latent_vec) in tqdm(enumerate(data_loader),total=total, desc="inference"):
        ...
        if args.mode == "diff":
            for i in range(bsz):
                new_vec = predict_vec[i] + cur_vec
                predict_vec[i] = cur_vec
                cur_vec = new_vec
        else:
