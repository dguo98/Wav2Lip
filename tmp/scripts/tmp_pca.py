...
if os.path.exists(f"./tmp/p_test_pca_inverse_n{n}/pca.pkl"):
...
with open(f"./tmp/p_test_pca_inverse_n{n}/pca.pkl", "wb") as f:
    pickle.dump(pca, f)
...
os.makedirs(f"./tmp/p_test_inverse", exist_ok=True)
os.makedirs(f"./tmp/p_test_pca_inverse_n{n}", exist_ok=True)
os.system(f"cp {data_dir}/audio.wav ./tmp/p_test_inverse/")
os.system(f"cp {data_dir}/audio.wav ./tmp/p_test_pca_inverse_n{n}/")

for i,vec in enumerate(vecs):
    torch.save(torch.tensor(vec+neutral), f"./tmp/p_test_inverse/{i:06d}.pt")
for i,vec in enumerate(inv_vecs):
    torch.save(torch.tensor(vec+neutral), f"./tmp/p_test_pca_inverse_n{n}/{i:06d}.pt")
...
for i in tqdm(range(n), "varying dims"):  # fix {i}-th dim
    ...
    os.makedirs(f"./tmp/p_test_pca_inverse_n{n}/{i}th", exist_ok=True)
    os.system(f"cp {data_dir}/audio.wav ./tmp/p_test_pca_inverse_n{n}/{i}th")
    for j,vec in enumerate(inv_vecs):
        torch.save(torch.tensor(vec+neutral), f"./tmp/p_test_pca_inverse_n{n}/{i}th/{j:06d}.pt")
"""
