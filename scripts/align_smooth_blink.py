...
for si in tqdm(range(32), desc="aligning"):
    ...
    save_dict={"eye_to_eye": eye_to_eye,
        "eye_avg": eye_avg, "mouth_avg": mouth_avg, "eye_to_mouth":eye_to_mouth, "eye_close": eye_close}
    ...
    while i+1 < len(lm):
        if eye_close[i] > cutoff:  # eye close
            j = i
            while j + 2 < len(lm) and eye_close[j+1] > cutoff:
                j += 1
            # [i, j]
            l = j-i+2  
            assert j+1<len(lm) and i-1>=0
